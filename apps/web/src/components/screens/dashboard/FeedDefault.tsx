...
export function FeedDefault({
  ...
  useEffect(() => {
    if (initialVideoId) {
      const matchedIdx = reels.findIndex(
        (item) => item.videoId === initialVideoId,
      );
      if (matchedIdx >= 0) {
        setCurrentIdx(matchedIdx);
      }
    }
  ...
  return (
    <AppShell activePage={activePage}>
      ...
      >
        {/* SECTION 1 — VIDEO */}
        <div
          className={`relative flex-[5] min-h-0 bg-gradient-to-br ${reel.bg} transition-colors duration-700`}
        >
          <div
          ...
          <div className="absolute right-3 sm:right-4 top-1/2 -translate-y-1/2 z-10 flex flex-col items-center gap-3">
            ...
            >
              <Zap
                className={`w-4 h-4 ${isMarked ? "text-white fill-white" : "text-white/80"}`}
              />
            </button>
        ...
        <div className="flex-[6] min-h-0 flex flex-col bg-[#F8F6F2] border-t-2 border-[#DDD9D2] overflow-y-auto">
          ...
          <div className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-[#DDD9D2] bg-[#EDEBE5] shrink-0">
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-bold text-[#958F87] uppercase tracking-widest">
                Transcript
              </span>
              <span className="text-[#D6D2C9]">·</span>
              <span className="text-[11px] text-[#958F87] hidden sm:inline">
                {reel.clipRange}
              </span>
            </div>
            <div className="flex items-center gap-2">
            ...
            {reel.lines.map((line, i) => {
              if (line.active) {
                ...
                return (
                  <div
                    key={i}
                    className="rounded-xl bg-[#E8E5DF] border border-[#DDD9D2] px-3 sm:px-4 py-3"
                  >
                    <div className="flex items-start gap-3">
                      <div className="w-6 h-6 rounded-full bg-[#E8E5DF] border border-[#DDD9D2] flex items-center justify-center shrink-0 mt-0.5">
                        <span className="text-[9px] font-bold text-[#52504B]">
                          {line.speaker[0]}
                        </span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <span className="text-[10px] font-bold text-[#958F87] mr-2">
                          {line.speaker}
                        </span>
                        ...
                        {showJa && (
                          <div className="flex items-start gap-1.5 mt-1.5">
                            <span className="text-[8px] font-bold text-[#958F87] shrink-0 mt-0.5 tracking-wide">
                              JA
                            </span>
              ...
              return (
                <div key={i} className="flex items-start gap-3 px-2 py-1">
                  <div className="w-6 h-6 rounded-full bg-[#E8E4DB] flex items-center justify-center shrink-0 mt-0.5">
                    <span className="text-[9px] font-bold text-[#9C9791]">
                      {line.speaker[0]}
                    </span>
                  </div>
                  <div>
                    ...
                    <p className="text-[#958F87] text-sm leading-snug mt-0.5">
                      {line.en as string}
                    </p>
                  </div>
                </div>
          ...
          <div className="px-4 sm:px-6 py-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-[#1C1917] rounded-2xl p-4 sm:p-5 text-white flex flex-col gap-3">
              ...
              <p className="text-white/70 text-sm leading-relaxed border-l-2 border-[#C8623E] pl-3">
                {reel.definition}
                {showJa && (
                  <span className="block text-white/38 text-xs mt-1 leading-relaxed">
                    {reel.definitionJa}
                  </span>
                )}
              </p>
              ...
              <div className="bg-white/5 rounded-xl p-3 border border-white/10">
                <strong className="text-white block mb-1 text-[10px] uppercase tracking-wider">
                  A native might say:
                </strong>
                <span className="text-white/55 italic text-xs leading-relaxed">
                  {reel.nativeUsage}
                </span>
              </div>
              ...
              >
                {isSaved ? (
                  ...
                  <>
                    <Bookmark className="w-4 h-4" /> Save &amp; study this
                    phrase
                  </>
                )}
              </button>
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#A09890] mt-0.5">
                      {reel.progressGoal - reel.progressDone} more to reach your
                      daily goal.
                    </p>
                  </div>
                </div>
              ...
              <div className="bg-white border border-[#DDD9D2] rounded-2xl p-4">
                <div className="flex items-center gap-1.5 mb-3">
                  ...
                  <span className="text-[10px] font-bold text-[#958F87] uppercase tracking-wider">
                    Related words
                  </span>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {reel.relatedWords.map((w) => (
                    <span
                      key={w}
                      className="text-xs px-2.5 py-1 rounded-full bg-[#F8F6F2] border border-[#DDD9D2] text-[#52504B] cursor-pointer hover:border-[#C5C0BB] hover:bg-[#EDEBE5] transition-colors"
                    >
                      {w}
                    </span>
                  ))}
            ...
            {showJa && (
              <div className="flex items-start gap-1.5 px-3 sm:px-4 py-3 rounded-xl bg-[#EDEBE5] border border-[#DDD9D2]">
                ...
                <div>
                  ...
                  <span className="text-[#52504B] text-xs font-semibold">
                    {reel.phraseJa}
                  </span>
                  <p className="text-[#958F87] text-xs mt-0.5 leading-relaxed">
                    {reel.definitionJa}
                  </p>
                </div>
              </div>
