...
const currentCard = {
  ...
  example:
    "We don't have much time, so let's cut to the chase: are we buying the company or not?",
...
export function Review() {
  ...
  const handleSubmit = () => {
    if (userAnswer.trim()) setPhase("ai_feedback");
  };
  const handleReveal = () => {
    setIsRevealed(true);
    setPhase("answered");
  ...
  const [scoreBg, scoreText, , barBg] =
    aiScore >= 85
      ...
      : aiScore >= 65
        ? ["bg-amber-300", "text-[#1C1917]", "border-amber-500", "bg-amber-500"]
  ...
  return (
    <AppShell activePage="review">
      <div className="max-w-xl mx-auto px-4 sm:px-6 py-6 sm:py-9">
        {/* Header */}
        <div className="mb-6 sm:mb-7 text-center">
          <h1
            className="text-2xl sm:text-3xl font-bold text-[#1C1917] mb-5 tracking-tight"
          ...
          <div className="inline-flex items-center gap-3 bg-[#1C1917] text-white px-4 py-2.5 rounded-xl border-2 border-[#1C1917] shadow-[3px_3px_0_#6B4226] mb-3">
            ...
            <div className="w-24 sm:w-32 h-2.5 bg-white/20 rounded-full overflow-hidden">
              <div
                className="h-full bg-amber-300 rounded-full"
                style={{ width: `${pct}%` }}
              />
            </div>
            <span className="text-lg font-bold tabular-nums font-mono text-white/60">
              {totalCount}
            </span>
          </div>
          <p className="text-[10px] text-[#A09890] uppercase tracking-wider font-bold">
        ...
        <div className="flex justify-center mb-5">
          {currentCard.reviewType === "AI_EVAL" ? (
            <span className="inline-flex items-center gap-2 text-xs font-bold px-3.5 py-2 rounded-lg border-2 border-[#1C1917] bg-violet-300 text-[#1C1917] shadow-[2.5px_2.5px_0_#1C1917]">
              <Brain className="w-3.5 h-3.5" /> AI Evaluation — write a
              sentence, get scored
            </span>
          ) : (
            <span className="inline-flex items-center gap-2 text-xs font-bold px-3.5 py-2 rounded-lg border-2 border-[#1C1917] bg-sky-300 text-[#1C1917] shadow-[2.5px_2.5px_0_#1C1917]">
              <RotateCcw className="w-3.5 h-3.5" /> Recall Mode — can you
              remember?
            </span>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <div
              ...
              style={{
                backgroundImage:
                  "radial-gradient(circle, #DDD9D2 1px, transparent 1px)",
                backgroundSize: "18px 18px",
              }}
            >
              <div className="h-2 w-full bg-[#C8623E]" />

              <div className="px-5 sm:px-7 pt-6 sm:pt-7 pb-7 sm:pb-8 bg-white/90">
                {/* Context sentence */}
                <div className="text-center mb-6">
                  ...
                  <p className="text-lg sm:text-xl text-[#1C1917] leading-relaxed font-light">
                ...
                <div className="flex items-center justify-center gap-2 text-xs text-[#A09890] mb-5">
                  <div className="w-7 h-5 rounded border border-[#DDD9D2] bg-[#E8E5DF] flex items-center justify-center">
                    <span className="text-[7px] font-bold text-[#6B6660]">
                      ▶
                    </span>
                  </div>
                  <span>
                    From:{" "}
                    <span className="font-bold text-[#52504B]">
                      {currentCard.sourceClip}
                    </span>
                  </span>
                  <button className="text-[#A09890] hover:text-[#C8623E] transition-colors ml-1">
                ...
                {phase === "ai_feedback" && (
                  <div className="bg-[#F8F6F2] rounded-xl p-4 border-2 border-[#DDD9D2]">
                    <p className="text-[9px] font-bold text-[#A09890] uppercase tracking-wider mb-1.5">
                      Your answer
                    </p>
                ...
                {currentCard.reviewType === "RECALL" && isRevealed && (
                  <div className="mt-5 pt-5 border-t-2 border-dashed border-[#DDD9D2] space-y-3">
                    <div>
                      <p className="text-[9px] font-bold uppercase tracking-wider text-[#A09890] mb-1">
                        Meaning
                      </p>
                    ...
                    <div className="bg-amber-50 p-4 rounded-xl border-2 border-amber-200">
                      ...
                      <p className="text-sm text-[#52504B] italic">
                        "{currentCard.example}"
                      </p>
                    </div>
                    <span className="inline-block font-mono text-xs bg-[#E8E5DF] text-[#52504B] px-2 py-1 rounded border border-[#DDD9D2]">
          ...
          {phase === "ai_feedback" && (
            <div
              className={`rounded-2xl border-2 border-[#1C1917] p-4 sm:p-5 ${scoreBg} ${scoreText} shadow-[4px_4px_0_#1C1917]`}
            >
          ...
          {/* Coming up queue */}
          <div>
            <p className="text-[9px] font-bold uppercase tracking-widest text-[#A09890] text-center mb-3">
              Coming up
            </p>
