...
const clips = [
  {
    ...
    mobileTags: ["Comedy", "Business", "Tech"],
    transcript: [
      {
        speaker: "Michael",
        text: "I was talking to Jan about the new filing system, and she completely blew me off.",
      },
      {
        speaker: "Jim",
        text: "Well, you kind of threw her under the bus in that meeting yesterday. What did you expect?",
        highlight: "threw her under the bus",
      },
    ],
  },
  {
...
function StickerTag({
  ...
  return (
    <span
      className={`inline-flex items-center font-bold rounded border border-[#1C1917] ${bg} ${textColor} ${sz} shadow-[1px_1px_0_#1C1917]`}
    >
      {children}
    </span>
  );
...
export function Feed() {
  ...
  const [mobileTopic, setMobileTopic] = useState("All");
  ...
  const toggleTopic = (label: string) =>
    setActiveTopics((p) =>
      p.includes(label) ? p.filter((t) => t !== label) : [...p, label],
    );
  ...
  const openInReel = (clip: (typeof clips)[number]) => {
    router.push(`/reel?videoId=${encodeURIComponent(clip.videoId)}&autoplay=1`);
  };
  const mobileCategories = ["All", "Comedy", "Business", "Tech", "Podcasts"];
  const mobileClips =
    mobileTopic === "All"
      ? clips
      : clips.filter((clip) => clip.mobileTags?.includes(mobileTopic));
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-8 py-6 sm:py-9">
        <div className="lg:hidden pt-1 pb-2">
          <div className="flex items-baseline gap-2 mb-4">
            <p
              className="text-[22px] leading-none font-medium text-[#1C1917] tracking-tight"
              style={{ fontFamily: "Inter, sans-serif" }}
            >
              Good morning, Felix
            </p>
            <span className="text-[14px] leading-none text-[#A09890]">
              · {mobileClips.length} clips today
            </span>
          </div>

          <div className="flex items-center gap-2 overflow-x-auto pb-3 -mx-4 px-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {mobileCategories.map((category) => {
              const isActive = mobileTopic === category;
              return (
                <button
                  key={category}
                  type="button"
                  onClick={() => setMobileTopic(category)}
                  className={`shrink-0 rounded-full border px-4 py-2 text-[13px] font-medium transition-colors ${
                    isActive
                      ? "bg-[#1C1917] text-white border-[#1C1917] shadow-[0_1px_0_rgba(0,0,0,0.08)]"
                      : "bg-white text-[#52504B] border-[#DDD9D2] hover:border-[#BFB7AE]"
                  }`}
                >
                  {category}
                </button>
          ...
          <div className="flex items-center justify-between mb-3 pt-1">
            <span className="text-[10px] font-bold uppercase tracking-[0.22em] text-[#B5B0A9]">
              Today's clips
            </span>
            {isSearchMode && (
              <button
                onClick={clearSearch}
                className="text-[12px] text-[#A09890] underline underline-offset-2"
              >
                clear search
              </button>
            )}
          ...
          <div className="space-y-4 max-w-[540px] mx-auto w-full">
            {mobileClips.map((clip) => (
              <button
                key={clip.id}
                type="button"
                onClick={() => openInReel(clip)}
                className="w-full text-left rounded-[22px] border border-[#DDD9D2] bg-white overflow-hidden shadow-[0_1px_0_rgba(0,0,0,0.03)] transition-transform active:scale-[0.99]"
              >
                <div className="relative aspect-[16/9] sm:aspect-[16/8.5] md:aspect-[16/8] bg-[#1D1917] flex items-center justify-center">
                  <div className="absolute inset-0 bg-gradient-to-br from-[#2A2623] via-[#1D1A18] to-[#161412]" />
                  <div className="absolute top-3 left-3 z-10">
                    <StickerTag
                      bg={clip.difficultyColor}
                      textColor={
                        clip.difficultyColor.includes("text-white")
                          ? "text-white"
                          : "text-[#1C1917]"
                      }
                      size="xs"
                  ...
                  <span
                    className="relative z-10 w-12 h-12 rounded-full bg-white/10 border border-white/30 flex items-center justify-center backdrop-blur-sm"
                    aria-hidden="true"
                  >
                    <Play className="w-5 h-5 ml-0.5 text-white fill-white" />
                  </span>
                  <div className="absolute bottom-3 right-3 z-10 flex items-center gap-1 text-[10px] text-white/75 font-mono">
                ...
                <div className="p-4 pb-4">
                  <div className="flex items-start justify-between gap-3 mb-2.5">
                    <div className="min-w-0">
                      <p className="text-[10px] font-bold text-[#B5B0A9] uppercase tracking-[0.22em] truncate">
                        {clip.showName} · {clip.start} – {clip.end}
                      </p>
                      <h3
                        className="mt-2 text-[18px] leading-tight font-semibold italic text-[#1C1917]"
                    ...
                    <Bookmark
                      className={`w-4 h-4 shrink-0 ${clip.isCompleted ? "text-[#C8623E] fill-[#C8623E]" : "text-[#B5B0A9]"}`}
                    />
                  </div>

                  <div className="flex items-center justify-between gap-3">
                    <StickerTag bg={clip.wordTypeBg} size="xs">
                      {clip.wordType}
                    </StickerTag>
                    <span className="inline-flex items-center gap-1.5 rounded-full border border-[#F0C7B7] px-3 py-1 text-[12px] font-medium text-[#C8623E] bg-[#FFF8F4]">
                      Watch clip <ChevronRight className="w-3.5 h-3.5" />
                    </span>
                  </div>
                </div>
              </button>
        ...
        <div className="hidden lg:block">
          <div className="mb-6 sm:mb-7 relative">
            ...
            <h1
              className="text-2xl sm:text-3xl font-bold text-[#1C1917] mb-1 tracking-tight"
              style={{ fontFamily: "Playfair Display, serif" }}
            >
              Daily Feed
            </h1>
          ...
          <div className="flex gap-2.5 mb-4">
            <form
              ...
              className="flex-1 flex items-center gap-2 bg-white border border-[#DDD9D2] rounded-lg px-3.5 py-2.5 focus-within:border-[#C8623E] transition-all"
            >
              ...
              {inputValue && (
                <button
                  type="button"
                  onClick={clearSearch}
                  className="text-[#A09890] hover:text-[#6B6660] shrink-0"
                >
          ...
          {showPrefs && (
            <div className="mb-4 bg-white border border-[#DDD9D2] rounded-lg p-4">
              <p className="text-[10px] font-bold uppercase tracking-widest text-[#A09890] mb-3">
                Filter by interest
              </p>
                ...
                {PREFERENCE_TOPICS.map((t) => {
                  ...
                  return (
                    ...
                    >
                      {t.label}
                      <span
                        className={`ml-1.5 font-normal text-[10px] ${active ? "text-white/60" : "text-[#A09890]"}`}
                      >
          ...
          <div className="flex items-center gap-2 mb-5 text-xs text-[#6B6660]">
            {isSearchMode ? (
              <>
                ...
                <StickerTag bg="bg-amber-200" size="sm">
                  "{query}"
                </StickerTag>
                <button
                  onClick={clearSearch}
                  className="text-[#A09890] underline underline-offset-2 hover:text-[#6B6660]"
                >
                  clear
                </button>
              </>
        ...
        <div className="hidden lg:grid lg:grid-cols-12 gap-6 lg:gap-7">
          {/* Clip list */}
              ...
              return (
                ...
                >
                  <div className="flex gap-3">
                    <div className="w-16 h-12 rounded-lg shrink-0 bg-[#E8E5DF] relative flex items-center justify-center overflow-hidden">
                      {clip.isCompleted ? (
                        ...
                        <Play
                          className={`w-4 h-4 ${isActive ? "text-[#C8623E]" : "text-[#A09890]"}`}
                        />
                      )}
                      <span className="absolute bottom-0.5 right-1 text-[8px] font-mono text-[#6B6660]">
                        {clip.duration}
                      </span>
                    </div>

                    <div className="flex-1 min-w-0 py-0.5">
                      <p className="text-[9px] font-bold text-[#A09890] uppercase tracking-wider truncate mb-0.5">
                        {clip.showName}
                      </p>
                      <h3
                        className={`text-xs font-semibold truncate mb-1.5 ${isActive ? "text-[#1C1917]" : "text-[#52504B]"}`}
                      >
                        {clip.title}
                      </h3>
                  ...
                  <div className="mt-2.5 pt-2 border-t border-dashed border-[#E8E5DF] flex items-center gap-2">
                    <BookOpen
                      className={`w-3 h-3 shrink-0 ${isActive ? "text-[#C8623E]" : "text-[#A09890]"}`}
                    />
                    <span
                      className={`text-[11px] flex-1 truncate ${isActive ? "text-[#1C1917] font-semibold" : "text-[#6B6660]"}`}
                    >
                      "{clip.targetWord}"
                    </span>
                    <StickerTag bg={clip.wordTypeBg} size="xs">
            ...
            <div className="bg-white rounded-xl border border-[#DDD9D2] overflow-hidden lg:sticky lg:top-8">
              ...
              <div className="px-4 sm:px-5 py-4 border-b border-[#E8E5DF]">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-[#A09890]">
                    Transcript
                  </span>
                ...
                {activeClip.transcript.length > 0 ? (
                  <div className="space-y-3">
                    {activeClip.transcript.map((line, i) => (
                      <div key={i}>
                        <span className="text-[9px] font-bold text-[#A09890] uppercase tracking-widest block mb-0.5">
                          {line.speaker}
                        </span>
                  ...
                  <p className="text-sm text-[#A09890] text-center py-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <span className="font-bold text-[#1C1917] text-sm">
                          "{activeClip.targetWord}"
                        </span>
                        <StickerTag bg={activeClip.wordTypeBg} size="xs">
                          {activeClip.wordType}
                        </StickerTag>
                      </div>
                      <p className="text-xs text-[#6B6660] leading-relaxed mb-3">
                        To betray a colleague or friend for personal gain,
                        usually to avoid blame.
                      </p>
                      <button
