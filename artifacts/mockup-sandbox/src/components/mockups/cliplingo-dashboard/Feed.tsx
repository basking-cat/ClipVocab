    ...
    <AppShell activePage="feed">
      <div className="max-w-5xl mx-auto px-8 py-9">
        ...
        <div className="flex gap-2.5 mb-4">
          <form onSubmit={handleSearch} className="flex-1 flex items-center gap-2 bg-white border border-[#DDD9D2] rounded-lg px-3.5 py-2 focus-within:border-[#C8623E] focus-within:ring-2 focus-within:ring-[#C8623E]/10 transition-all">
            <Search className="w-3.5 h-3.5 text-[#958F87] shrink-0" />
            <input
              type="text"
              value={inputValue}
              onChange={e => setInputValue(e.target.value)}
              placeholder="Search a word or phrase…"
              className="flex-1 bg-transparent text-[#1C1917] placeholder:text-[#958F87] text-sm outline-none"
            />
            {inputValue && (
              <button type="button" onClick={clearSearch} className="text-[#958F87] hover:text-[#52504B]">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3.5 py-2 rounded-lg border text-sm font-medium transition-all ${showPrefs ? 'border-[#C8623E] bg-[#FAF0EB] text-[#C8623E]' : 'border-[#DDD9D2] bg-white text-[#52504B] hover:border-[#B5B0A9]'}`}
          >
        ...
        {showPrefs && (
          <div className="mb-4 bg-white border border-[#DDD9D2] rounded-lg p-4">
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#958F87] mb-3">Filter by interest</p>
              ...
              {PREFERENCE_TOPICS.map(t => {
                ...
                return (
                  <button
                    ...
                    className={`text-xs font-semibold px-3 py-1.5 rounded border transition-all ${active ? 'bg-[#1C1917] text-white border-[#1C1917]' : 'bg-white border-[#DDD9D2] text-[#52504B] hover:border-[#B5B0A9]'}`}
                  >
                    {t.label}
                    <span className={`ml-1.5 font-normal ${active ? 'text-white/60' : 'text-[#958F87]'}`}>{Math.round(t.weight * 100)}%</span>
                  </button>
                );
              })}
            </div>
        ...
        {/* Mode tag */}
        <div className="flex items-center gap-2 mb-5 text-xs text-[#52504B]">
          {isSearchMode ? (
            <>
              <span className="font-semibold">{clips.length} clips for</span>
              <span className="bg-[#F5E2D8] text-[#C8623E] font-bold px-2.5 py-0.5 rounded">"{query}"</span>
              <button onClick={clearSearch} className="text-[#958F87] hover:text-[#52504B] underline underline-offset-2">clear</button>
            </>
          ) : (
            <span className="text-[#958F87]">{clips.length} clips today</span>
          )}
        </div>
        ...
        <div className="grid grid-cols-12 gap-7">
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-3.5 rounded-lg border transition-all ${isActive
                    ...
                    : 'bg-white border-[#DDD9D2] hover:border-[#B5B0A9] opacity-70 hover:opacity-100'}`}
                >
                  <div className="flex gap-3">
                    <div className={`w-16 h-12 rounded shrink-0 bg-[#E8E5DF] relative flex items-center justify-center`}>
                      {clip.isCompleted ? (
                        <div className="absolute inset-0 bg-[#1C1917]/50 rounded flex items-center justify-center">
                          <CheckCircle2 className="w-4 h-4 text-white" />
                        </div>
                      ) : (
                        <Play className={`w-4 h-4 ${isActive ? 'text-[#C8623E]' : 'text-[#958F87]'}`} />
                      )}
                    ...
                    <div className="flex-1 min-w-0 py-0.5">
                      <p className="text-[9px] font-bold text-[#958F87] uppercase tracking-wider truncate mb-0.5">{clip.showName}</p>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[9px] text-[#958F87] font-mono flex items-center gap-0.5"><Clock className="w-2 h-2" />{clip.start}–{clip.end}</span>
                      </div>
                    </div>
                  </div>
                  <div className={`mt-2.5 flex items-center gap-1.5 text-[11px] ${isActive ? 'text-[#C8623E]' : 'text-[#958F87]'}`}>
                    <BookOpen className="w-3 h-3" />
                    <span>Learn: <span className="font-semibold">"{clip.targetWord}"</span></span>
                    <span className={`text-[9px] px-1.5 py-0.5 rounded ${isActive ? 'bg-[#F5E2D8]' : 'bg-[#E8E5DF]'} ml-auto`}>{clip.wordType}</span>
                  </div>
                </button>
              );
            })}
          ...
          <div className="col-span-7">
            <div className="bg-white rounded-lg border border-[#DDD9D2] overflow-hidden sticky top-8">
              ...
              <div className="px-5 py-4 border-b border-[#E8E5DF]">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-[#958F87]">Transcript</span>
                  <span className="text-[10px] text-[#958F87] font-mono">{activeClip.start} – {activeClip.end}</span>
                </div>
                {activeClip.transcript.length > 0 ? (
                  <div className="space-y-3">
                    {activeClip.transcript.map((line, i) => (
                      <div key={i}>
                        <span className="text-[9px] font-bold text-[#958F87] uppercase tracking-widest block mb-0.5">{line.speaker}</span>
                            ...
                            <>
                              {line.text.split(line.highlight).map((part, j, arr) => (
                                <React.Fragment key={j}>
                                  {part}
                                  {j < arr.length - 1 && (
                                    <mark className="not-italic bg-[#F5E2D8] text-[#C8623E] font-semibold px-1 py-0.5 rounded border-b-2 border-[#C8623E]">
                                      {line.highlight}
                                    </mark>
                                  )}
                                </React.Fragment>
                  ...
                  <p className="text-sm text-[#958F87] text-center py-3">No transcript for this clip</p>
                )}
              </div>

              {/* Word card */}
              <div className="px-5 py-4">
                <div className="border border-[#DDD9D2] rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 rounded bg-[#F5E2D8] flex items-center justify-center shrink-0">
                      <BookOpen className="w-4 h-4 text-[#C8623E]" />
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-semibold text-[#1C1917] text-sm">"{activeClip.targetWord}"</span>
                        <span className="text-[9px] font-bold bg-[#E8E5DF] text-[#52504B] px-1.5 py-0.5 rounded">{activeClip.wordType}</span>
                      </div>
                      <p className="text-xs text-[#52504B] leading-relaxed mb-3">To betray a colleague or friend for personal gain, usually to avoid blame.</p>
                      <button
                        onClick={() => setWordSaved(s => !s)}
                        className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-semibold transition-all ${wordSaved ? 'bg-[#F5E2D8] text-[#C8623E] border border-[#C8623E]/30' : 'bg-[#1C1917] text-white hover:bg-[#282622]'}`}
                      >
